...
async function createAssessment(token) {
    const projectId = "prompt-war-492920";
    const siteKey = "6Ld7pb4sAAAAAKdIn1hceH9fxd9iJj03LORY5yWn";
...
// Persistence is now handled by Firestore collections: 'users', 'bookings', 'archivedBookings'

const GOOGLE_CLIENT_ID = "666422970821-g87nbhojue4h457h8rp14cjt781reh63.apps.googleusercontent.com";
...
const requireAdmin = async (req, res, next) => {
    const userDoc = await db.collection('users').doc(req.userId).get();
    if (userDoc.exists && userDoc.data().role === "ADMIN") next();
    else res.status(403).json({ error: "Forbidden: Admin only" });
>>>>>>> 51dbaa4 (Initial commit)
};
