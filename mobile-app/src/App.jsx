...
<<<<<<< HEAD
import { Map, Bell, User, Ticket, CreditCard, X, ShieldAlert, Navigation, Settings2, MessageSquareWarning, MapPin } from 'lucide-react';
=======
...
>>>>>>> 51dbaa4 (Initial commit)
  ...
  const openCheckout = () => {
<<<<<<< HEAD
      // Photo requirement removed manually per client specifications
      const initialPassengers = selectedSeats.map(seat => ({
          seat, name: '', age: '', gender: 'Male', contact: '', address: '' 
      }));
...
<<<<<<< HEAD
    try {
      const payloadBody = { venueId: "venue-1", passengers: passengers, cardDetails: { number: cardNum } };
...
<<<<<<< HEAD
      ...
      try {
          ...
          if (res.ok) alert("Zone Manually Updated. UI dynamically routing users!");
      } catch(e) {}
  }
                     ...
                     <div key={z.id} className="glass-panel" style={{ background: bgColor, borderLeft }}>
                       <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                         ...
                         <div style={{flex: 1}}>
                           ...
                           <p className="subtitle" style={{marginBottom:'6px'}}>Approx. clearance wait time: <b>{z.waitTime} minutes</b>.</p>
                           ...
                           {z.congestionLevel === 'high' && betterAlternative && (
                               <p style={{fontSize: '0.8rem', color: 'var(--accent-blue)', marginTop: '4px'}}>
                                   💡 Suggestion: Head to <b>{betterAlternative.name}</b> (only {betterAlternative.waitTime} min wait!)
                               </p>
                           )}
                         </div>
                ...
                bookings.map((b) => (
                  <div key={b.id} className="glass-panel" style={{display: 'flex', alignItems: 'flex-start', gap: '12px'}}>
                    ...
                    <div style={{display: 'flex', flexDirection: 'column'}}>
                       ...
                       <span className="subtitle" style={{marginBottom:'4px'}}>{b.name} (PII Decrypted View)</span>
                       <span style={{fontSize:'0.8rem', color:'var(--status-low)'}}>Optimal Gate: {b.nearestGate}</span>
                    </div>
                  </div>
                ))
=======
